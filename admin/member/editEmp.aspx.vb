...
Partial Class admin_member_editEmp
    ...
    Sub updateProfile()
        ...
        Dim strUpdate As String = "UPDATE Lecturer SET FirstName =@fname, " & _
        "LastName =@lname, Address =@addr, Province =@prov, City =@city, District =@dist, " & _
        "Postcode =@post, Tel =@tel, Mobile =@mob, Email =@email,Qual_degree=@qual_degree,Qual_fromUni=@qual_fromUni," & _
        "Qual_branch=@qual_branch,Salary=@salary,Position=@position,Comfirm=@comfirm WHERE Username = @user"
        ...
        param.AddWithValue("@qual_degree", ddlDegree.SelectedValue)
        param.AddWithValue("@qual_fromUni", txtFromUni.Text)
        param.AddWithValue("@qual_branch", txtBranch.Text)
        param.AddWithValue("@salary", txtSalary.Text)
        param.AddWithValue("@position", ddlPosition.SelectedValue)
        param.AddWithValue("@comfirm", empActive)
    ...
    Protected Sub GridView2_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridView2.SelectedIndexChanged
        ...
        If Not String.IsNullOrEmpty(user) Then
            Dim dt As DataTable = DBClass.SqlGet("SELECT * FROM Lecturer WHERE UserName='" & user & "'", "getMember").Tables(0)
            ...
            ddlDegree.SelectedValue = dt.Rows(0)("Qual_degree")
            txtFromUni.Text = dt.Rows(0)("Qual_fromUni")
            txtBranch.Text = dt.Rows(0)("Qual_branch")
            txtSalary.Text = dt.Rows(0)("Salary")
            ddlPosition.SelectedValue = dt.Rows(0)("Position")
            If dt.Rows(0)("Comfirm") = True Then
